...
public class FrmManteProd extends JFrame {
	...
	void llenaCombo() {
		...
		EntityManagerFactory conexion = Persistence.createEntityManagerFactory("jpa_sesion01");
		 ...
		 List<Categoria > lsCategorias =en.createQuery("select c from Categoria c ").getResultList();
		 cboCategorias.addItem("Seleciones");
	...
	void listado() {
		...
		EntityManagerFactory conexion = Persistence.createEntityManagerFactory("jpa_sesion01");
		 ...
		 List<Producto > lsProductos =en.createQuery("select c from Categoria c ").getResultList();
		 txtSalida.setText("");
		 
		 for (Producto p c : lsProductos) {
			 
			txtSalida.append(p.getDes_prod()+ "" + p.getDes_prod()+""+pgetPre_prod + ""+ p.getIdcategoria()+ "")
		 }
		 em.close();
